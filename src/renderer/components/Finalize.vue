...
import {mugleNode, mugleLocalNode} from '../../shared/config'
  ...
  methods: {
    ...
    send(){
      ...
      let finalize = async function(){
        ...
        this.$log.debug('check mugle local status before finalize: ' + localGnodeStatus)
        if(localGnodeStatus == 'running'){
          gnode = mugleLocalNode
        }
        this.$log.debug('finalize use mugle node:' + gnode)
