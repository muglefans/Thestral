...
import {chainType, mugleNode2, mugleLocalNode, chain} from '../../shared/config'
...
export default {
  ...
  created(){
    messageBus.$on('walletRecoverReturn', (ret)=>{
      ...
      if(ret === 'ok'){
        let gnode = mugleNode2

        this.page = 'recovered'
        this.$log.debug('check mugle local status before restore balance: ' + this.localGnodeStatus)
        if(this.localGnodeStatus == 'running'){
          this.$log.debug('check use mugle local node')
          gnode = mugleLocalNode
        }
  ...
  methods: {
    ...
    updateOutput(data){
      //let toDel = 'mugle_wallet_libwallet::internal::restore'
