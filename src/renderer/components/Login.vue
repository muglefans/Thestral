...
import {version, mugleNode, gnodeOption, mugleNode2, mugleLocalNode, platform, chain} from '../../shared/config'
  ...
  methods: {
    ...
    selectGnode(){
      ...
      if(!gnodeOption.useLocalGnode){
        ...
        this.$log.debug('use remote mugle node.')
        this.gnode = mugleNode
        this.readyToLogin()
        return
      }
      ...
      if(gnodeOption.connectMethod ==='localAllTime'){
        ...
        this.$log.debug('use local mugle node.')
        this.gnode = mugleLocalNode
        this.readyToLogin()
        return
      }
      ...
      this.$remoteGnodeService.getStatus().then((res)=>{
        ...
        this.$gnodeService.getStatus().then((res)=>{
          ...
          if(localHeight + 60 >= remoteHeight && peersCount >= 3){
            ...
            this.$log.debug('local node height is too low or peers too small. use remote mugle node.')
            this.$dbService.setGnodeLocation('remote')
            this.gnode = mugleNode
            this.readyToLogin()
            return
          }}).catch((err)=>{
            this.$log.error('local gnode failed. Use remote mugle node: ' + err)
              this.$dbService.setGnodeLocation('remote')
              this.gnode = mugleNode
              this.readyToLogin()
              return
    ...
    tryLogin(){
      ...
      this.logining = true
      //this.$walletService.killMugleWallet()
