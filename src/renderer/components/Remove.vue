...
import {walletDataPath, walletLogPath, walletTOMLPath, thestralPath} from '../../shared/config'
  ...
  methods: {
    ...
    remove(){
      ...
      if(['移除', 'remove'].indexOf(rcText) >= 0){
        ...
        //let dest = path.join(thestralPath, `.mugle_${suffix}`)
        //fse.moveSync(mugleDIR, dest)
