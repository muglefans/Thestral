...
import {hedwigServer, hedwigClient, hedwigApp, mugleNode, mugleLocalNode} from '../../shared/config'
...
export default {
  ...
  methods: {
    start(){
      let gnode = mugleNode
      if(this.$dbService.getGnodeLocation() == 'local')gnode=mugleLocalNode
      if(!this.starting&&!this.internetReachable){
        ...
        this.checklocalReachable().catch((error)=>{
          ...
          if(!this.internetReachable){
            let args = ['--server', hedwigServer, '--app', hedwigApp, '--port', '6815', 
