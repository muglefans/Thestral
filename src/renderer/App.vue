            ...
            <div class="level">
             ...
             <div class="level-right">
              <p class="tag is-warning animated level-item" v-bind:class="{headShake: isAnimate}" style="animation-iteration-count:3; margin-right: 0.4em">
                {{ $t("msg.app.height") }} ({{getGnodeLocationDisplay()}}): {{height}} / {{latestHeight}}</p>
    ...
    methods: {
      ...
      getLatestHeight(){
        this.$gnodeService.getConnectedPeers().then(
          (res)=>{
          let peers = res.data.result.Ok
          let tLatestHeight = this.height;
          for (let i=0;i<peers.length;i++){
            let itemHeight = parseInt(peers[i].height)  
            if (itemHeight > tLatestHeight) {
                  tLatestHeight = itemHeight;
            }
          }
          this.latestHeight = tLatestHeight;
        }).catch(
          (error)=>{
