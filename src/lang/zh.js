const messages = {
  msg: {
    ...
    wrongPassword: 'Mugle owner Api进程启动失败，可能是密码错误',
    login_: '登录',
    logout: '登出',
    search: '搜索',
    clearup: '清除',
    ...
    login: {
      walletExist: '发现已有Mugle钱包存在；请用原有的密码登陆吧 :-)',
      waitForLocalGnode: '等待本地Mugle节点启动中',
      selectingGnode: '正在选择Mugle节点',
      gnode: 'Mugle节点'
    },
    ...
    remove:{
      title: '移除当前钱包',
      warning: '警告 !',
      info: '移除钱包前，请确认 当前钱包里已经没有mugle 或者 你已经完整记录了当前钱包的助记词 !',
      help: '在下面输入框填入 “移除” 用于确认',
      remove: '移除钱包',
      success: '当前钱包已经移除，请重启Thestral'
    },
    ...
    restore:{
      ...
      recover: '恢复钱包',
      reAdd: '重新输入',
      recovered: '钱包恢复成功, 开始从Mugle区块链同步钱包余额；',
    ...
    app:{
      ...
      height: '同步高度',
      updateTitle: '发现新版本',
      updateMsg: '发现Thestral钱包的新版本, 请立刻更新! (下载并覆盖原有程序即可).',
      yes: '好的',
      no: '暂时不更新',
    ...
    httpReceive: {
      ...
      close: '关闭HTTP监听',
      attention: '注意',
      reachableMsg: '设置HTTP接收Mugle: 需要你有对外的公网ip, 并且你的ip能被访问',
      password: '钱包密码 (用于启动http监听)',
      start: '启动',
      error: '没有输入密码.',
      failed: '启动失败，可能是密码错误',
      failed2: '监听失败，你的对外ip无法被访问到; 请使用交易文件的方法 或者 Hedwig 来收发Mugle.',
      failed3: '无法获取你的IP地址，请过段时间再试下',
      failed4: '本地HTTP监听(http://127.0.0.1:6815)已经启动。但是你的对外ip无法被访问到; 请使用交易文件的方法 或者 Hedwig 来收发Mugle.',
      ip: '你的公网IP'
    },
    ...
    hedwig: {
      ...
      intro2: '当有人向这个临时地址发送mugle时，它会把这个发币请求转发到你的Thestral钱包，完成交易.',
      start: '启动',
      failed: '连接到hedwig服务器时 发生错误，等下再试吧.',
      failed2: '连接到hedwig临时地址时 发生错误， 重启下钱包 or 等下再试吧.',
      failed3: '启动本地mugle接收服务失败， 重启下钱包 or 等下再试吧.',
      copy: '复制地址',
      copied: '地址已经复制到剪贴板'
    },
    ...
    check: {
      ...
      intro1: '当你的本地钱包余额 和 Mugle区块链上的余额不一致(可能是由于交易被取消或者发送失败等原因)时，可以使用 "同步余额" 功能.',
      intro2: '同步时，在Mugle区块链的所有未花费的输出(uxtos)中，找到你可以使用的未花费的输出，相加就得到你的余额.',
    ...
    gnode:{
      ...
      connectedCount: '已连接到周边Mugle节点数量',
      location: '区块数据存储的位置',
      size: '全节点区块数据大小',
      restart: '重启本地节点',
      height: '最新高度',
      remove: '移除区块数据，重头开始同步',
      removeConfirm: '确认移除目前的Mugle区块数据吗？'
    },
    ...
    tor:{
      ...
      intro2: '通过洋葱网络接收/发送 mugle， 安全/匿名/去中心化。',
    ...
    gaddress:{
      ...
      copied: '已复制',
      copy: '复制 slatepack地址', 
      tips: 'slatepack 地址 用于加密slatepack信息以及付款证明，并不会存储在Mugle区块链上.'
    },

    log: {
      title: '导出日志',
      saveMsg: '导出当天的Thestral钱包的日志',
    }
